...
#reader(lib "htdp-intermediate-reader.ss" "lang")((modname |Pset 11 draft|) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
(define (playlist-combiner title lop)
  (local [(define (grab-videos pl base)
            (append (playlist-videos pl) base))]
    ; append appends combines 2 lists
...
(define (list-builder-helper lon list-count k)
  (cond [(empty? lon) lon]
        [(cons? lon) (if (= (- (length lon) 1) list-count)
                         (append (list k) (list-builder-helper (rest lon) list-count k))
                         (append (list 0) (list-builder-helper (rest lon) list-count k)))]))
#; ;attempt 2
(define (list-builder-helper lon list-count k)
  (cond [(empty? lon) lon]
        [(cons? lon) (if (= (length lon) list-count)
                         (cons k (list-builder-helper (rest lon) list-count k) )
                         (cons 0 (list-builder-helper (rest lon) list-count k)))]))

;(list-builder-helper (list 1 2 3 4 5 6) 3 1)                                  
(define (list-builder-helper lon list-count k)
  (cond [(empty? lon) lon]
        [(cons? lon) (if (= (length lon) list-count)
...
; retrieves a value in a list at the index in a list at the index
