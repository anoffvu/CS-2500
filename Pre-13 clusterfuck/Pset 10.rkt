...
; author-grab : InfoSource String -> String
; takes in an infosource and outputs a string appended to its title 
(check-expect (author-grab INFO-SOURCE/BOOK "") "David Mamet ")
(check-expect (author-grab INFO-SOURCE/BOOK "hello") "David Mamet hello")
(check-expect (author-grab INFO-SOURCE/ARTICLE "") "Kyle Buchanan ")
(check-expect (author-grab INFO-SOURCE/ARTICLE "hi") "Kyle Buchanan hi")
...
;ex 4
; total-pages : InfoList -> Number
; finds the total number of pages in an InfoList
(check-expect (total-pages INFOLIST-0) 141)
(check-expect (total-pages INFOLIST-1) 128)
(check-expect (total-pages INFOLIST-2) 13)
(check-expect (total-pages INFOLIST-3) 0)

(define (total-pages il)
  (foldr add-pages 0 il))

; add-pages : InfoSource Number -> Number
; adds the pages in an InfoSource to a Number
(check-expect (add-pages INFO-SOURCE/BOOK 0) 128)
(check-expect (add-pages INFO-SOURCE/BOOK 10) 138)
(check-expect (add-pages INFO-SOURCE/ARTICLE 0) 13)
(check-expect (add-pages INFO-SOURCE/ARTICLE 100) 113)

(define (add-pages is num)
  (cond [(book? is) (+ (book-page-count is) num)]
        [(article? is) (+ (ceiling (/ (article-word-count is) 100)) num)]))


;ex 5
; use abstracted functions initally, so I'm not sure how we abstract those functions any more?
